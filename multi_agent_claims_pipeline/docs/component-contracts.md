...
| Decision precedence | A fixed, deterministic sequence — never LLM-chosen. In order: (1) if `policy_evaluation_result` or `financial_calculation_result` is `None` → `MANUAL_REVIEW` (`MANUAL_REVIEW_INSUFFICIENT_EVIDENCE`, `approved_amount=None`); (2) claim-level (non-itemized) policy exclusion, waiting period, and pre-authorization-missing are each checked and **collected together** (not first-match-wins) — any one or more present → `REJECTED` with every applicable `RejectionReason`; (3) if none of those fired and `fraud_analysis_result.requires_manual_review` → `MANUAL_REVIEW` (`MANUAL_REVIEW_FRAUD`, but still surfaces `financial.payable_amount` since it's reliable); (4) if `financial.payable_amount <= 0` → `REJECTED` (`REJECTED_ZERO_PAYABLE`); (5) otherwise `PARTIAL` if `financial.eligible_amount < financial.claimed_amount` (genuine line-item exclusion — DENTAL/VISION only) else `APPROVED`; (6) if overall confidence is below `_LOW_CONFIDENCE_THRESHOLD` (0.5) even at this point, downgrade to `MANUAL_REVIEW` (`MANUAL_REVIEW_LOW_CONFIDENCE`) while still keeping the reliable `approved_amount`. Full derivation from `assignment.md`/`test_cases.json`/`policy_terms.json`, including the one official test case (TC008) whose expected decision this precedence deliberately does not reproduce, and why, in `docs/tradeoffs.md` "Decision Precedence". |
