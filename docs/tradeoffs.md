...
### Every Document Must Be Identifiable in Cross-Document Identity Validation

**Choice**: `CrossDocumentValidationAgent` used to treat *one* document with a legible, member-matching patient name as enough to pass the whole claim — any other real document with no readable name at all rode along unverified, as long as it didn't actively disagree with the one that was readable. That's backwards for an insurance claim: the document with no visible patient name is exactly the one whose payable amount is about to be approved, and "some other document had a name" doesn't establish that *this* one belongs to this member. This was first fixed for the total-absence case (zero documents readable at all — silently PASSED on "nothing to cross-check", now BLOCKS), then generalized: when a real classification pass (`DocumentClassification.source == "ai"`) finds even *one* document with no legible name and the claim's member is known, the claim BLOCKS naming which document couldn't be confirmed — the same conservative "can't verify -> don't assume" stance already used for network-hospital matching above. A detected identity *conflict* (documents disagreeing with each other, or agreeing on the wrong person) is still reported first and takes priority over "some document has no name", since a specific wrong-identity finding is more actionable than a generic "can't confirm every document" one.

**Reason**: the existing document<->member check (added by the earlier identity-fix, see `app/agents/cross_document_validation_agent.py`) only ever ran against the *named* documents — an unnamed document was invisible to it, never actually verified, which is the opposite of what "we can't confirm this document" should mean when real money is being paid out against it.

**Trade-off**: evaluation fixtures (`DocumentClassification.source == "fixture"`, from `DocumentInputAdapter`) are deliberately exempt — `test_cases.json` only populates `patient_name_on_doc` for the one official case that specifically tests identity matching (TC003), so blanket enforcement would have blocked most of the other eleven official cases for a reason unrelated to what they're actually testing. This means the stricter check only actually fires on the real upload path (`POST /api/v1/claims`), not the official evaluation — intentional, since the official evaluation was never exercising real OCR/AI name extraction in the first place. See `tests/unit/test_cross_document_validation_agent.py::TestNoLegibleNameGapFix` / `::TestEveryDocumentMustBeIdentifiable` and `tests/integration/test_claims_api.py::test_submit_claim_with_no_legible_patient_name_blocks_instead_of_silently_passing` / `::test_submit_claim_with_one_unnamed_document_blocks_even_if_other_matches`.
