...
| Output | `FinancialBreakdown` (`app/domain/models.py`, extended in Phase 2C) — `claimed_amount`, `eligible_amount`, `network_discount_percent`/`_amount`, `amount_after_network_discount`, `sub_limit`/`sub_limit_applied`, `per_claim_limit`/`per_claim_limit_applied`, `annual_opd_limit`/`annual_limit_applied`, `amount_after_limits`, `copay_percent`/`_amount`, `payable_amount`, `currency` ("INR"), `calculation_steps: List[str]` (human-readable audit trail, one entry per step actually applied), `warnings: List[str]` (reconciliation mismatches — see below), `confidence` (1.0, or 0.7 if any warning was recorded). `sub_limit_applied`/`per_claim_limit_applied` are always `False` (Phase 3 — see next row); the fields are kept for API/frontend backward compatibility and to surface the configured limit values, not because either is ever applied here. |
| Calculation order | claimed/itemized-eligible amount → network discount → remaining annual OPD allowance cap (`max(annual_opd_limit - ytd_claims_amount, 0)`) → copay deduction → `payable_amount`. `sub_limit` and `per_claim_limit` are **not** applied as caps here (Phase 3 correctness fix) — `test_cases.json`'s own official TC006/TC010 worked amounts both pay out the full discount/copay-adjusted figure despite exceeding these limits. `per_claim_limit` is instead a whole-claim REJECT gate evaluated by `DecisionGenerationAgent` before any payable figure is used for an approval — see `docs/tradeoffs.md` "Phase 3 Correctness Pass" for the full derivation and `docs/eval-report.md` for the resulting 12/12 official-case match. |
...
| Trace behavior | One `FINANCIAL_CALCULATION` `STARTED`→`COMPLETED`/`FAILED`/`SKIPPED` pair. Metadata: `payable_amount`, `sub_limit_applied`/`per_claim_limit_applied` (always `False` — see above)/`annual_limit_applied`, `warnings_count` — the full `calculation_steps` audit trail lives on the persisted result, not duplicated into the trace row. |
...
## DecisionGenerationAgent (Phase 2D)

**File**: `app/agents/decision_generation_agent.py`

### Contract

| Property | Value |
|----------|-------|
| Purpose | The deterministic authority on the final claim decision — `APPROVED`/`PARTIAL`/`REJECTED`/`MANUAL_REVIEW`. Makes **zero AI calls** (`ai_provider=None`, same pattern as `ClaimValidationAgent`/`PolicyEngine`/`FraudAnalysisAgent`) and performs **no independent policy or financial calculation** — it only combines the already-computed outputs of `PolicyEngine`, `FinancialCalculationService`, and `FraudAnalysisAgent` (Phase 2C) plus `DocumentExtractionAgent`/`DocumentVerificationAgent`/`CrossDocumentValidationAgent` (Phase 2A/2B), all read off the same `Claim` object those stages already populated. |
| Input | `run(claim: Claim) -> ClaimDecision`. No separate `DecisionGenerationInput` DTO — `Claim` already aggregates every result this agent needs; a parallel wrapper model would just duplicate `Claim`'s own fields (same reasoning `PolicyEngine.evaluate(claim)`/`FraudAnalysisAgent.run(claim)` already established). |
| Output | `ClaimDecision` (`app/domain/models.py` — a Phase 0 placeholder, unused until now, deliberately reused rather than replaced with a new Phase 2D-specific shape). Two fields were added in Phase 2D: `reason_code: Optional[str]` (short machine-readable code, e.g. `"REJECTED_WAITING_PERIOD"`, `"MANUAL_REVIEW_FRAUD"` — see the full vocabulary below) and `degraded_components: List[str]` (which upstream stages were missing/failed, driving both the confidence penalty and `manual_review_recommended`). Every other field (`decision`, `approved_amount`, `rejection_reasons`, `line_item_decisions`, `confidence_score`, `explanation`, `member_facing_message`, `has_component_failures`, `manual_review_recommended`, `fraud_signals`, `decided_at`) already existed. |
| Decision precedence | A fixed, deterministic sequence — never LLM-chosen. In order: (1) if `policy_evaluation_result` or `financial_calculation_result` is `None` → `MANUAL_REVIEW` (`MANUAL_REVIEW_INSUFFICIENT_EVIDENCE`, `approved_amount=None`); (2) claim-level (non-itemized) policy exclusion, waiting period, and pre-authorization-missing are each checked and **collected together** (not first-match-wins) — any one or more present → `REJECTED` with every applicable `RejectionReason`; (3) (Rule 5.5, Phase 3) if none of those fired, the claimed amount exceeds `per_claim_limit`, and the claim has no line-item-driven partial eligibility (`eligible_amount == claimed_amount`) → `REJECTED` (`PER_CLAIM_EXCEEDED`); (4) if none of the above fired and `fraud_analysis_result.requires_manual_review` → `MANUAL_REVIEW` (`MANUAL_REVIEW_FRAUD`, but still surfaces `financial.payable_amount` since it's reliable); (5) if `financial.payable_amount <= 0` → `REJECTED` (`REJECTED_ZERO_PAYABLE`); (6) otherwise `PARTIAL` if `financial.eligible_amount < financial.claimed_amount` (genuine line-item exclusion — DENTAL/VISION only) else `APPROVED`; (7) if overall confidence is below `_LOW_CONFIDENCE_THRESHOLD` (0.5) even at this point, downgrade to `MANUAL_REVIEW` (`MANUAL_REVIEW_LOW_CONFIDENCE`) while still keeping the reliable `approved_amount`. Full derivation from `assignment.md`/`test_cases.json`/`policy_terms.json` — including how Rule 3 reconciles TC006/TC008/TC010, previously a disclosed discrepancy, now an exact 12/12 match — in `docs/tradeoffs.md` "Decision Precedence" and "Phase 3 Correctness Pass". |
| Rejection reason mapping | Claim-level exclusion → `RejectionReason.EXCLUDED_CONDITION`; waiting period → `WAITING_PERIOD`; pre-auth missing → `PRE_AUTH_MISSING`; a DENTAL/VISION excluded line item → `LineItemDecision.rejection_reason = EXCLUDED_PROCEDURE` (per-item, not claim-level). |
| Approved amount | **Always** exactly `financial_calculation_result.payable_amount` (Decimal, never recomputed) for `APPROVED`/`PARTIAL`, and for `MANUAL_REVIEW` whenever a reliable financial result exists; `Decimal("0")` for `REJECTED`; `None` only when there is no reliable financial result at all (the insufficient-evidence path). Locked in by `TestDeterministicAmountPreserved` (`tests/unit/test_decision_generation_agent.py`). |
| Confidence | Deterministic and **operational, not statistically calibrated** — see `docs/tradeoffs.md` "Decision Confidence Strategy" for the exact formula (minimum confidence across every stage that ran, minus a penalty per missing/degraded stage). |
| Failure handling | The agent itself is pure Python with no I/O, so it should never raise in practice — but `ClaimsPipeline` still wraps the call defensively: if it somehow does raise, `_fallback_decision()` constructs a conservative `MANUAL_REVIEW` decision (`reason_code="MANUAL_REVIEW_DECISION_GENERATION_FAILED"`) rather than leaving `claim.decision` `None` — unlike Policy/Financial/Fraud's soft-fail-to-`None` pattern, a `None` decision here would mean the API has nothing to return for "what happened to my claim," which assignment.md point 4 requires to always exist once a claim reaches this stage. |
| Trace behavior | One `DECISION_GENERATION` `STARTED`→`COMPLETED`/`FAILED` pair. Metadata: `decision`, `approved_amount`, `confidence`, `reason_code`, `manual_review_required`, `degraded_components` — never the full findings/evidence (that's on the persisted `ClaimDecision` itself). |

---

## ExplanationAgent (Phase 2D)
...
| Property | Value |
|----------|-------|
| Purpose | Turns the already-finalised `ClaimDecision` (produced by `DecisionGenerationAgent`, never touched or second-guessed) plus its supporting evidence into a structured, human-readable explanation, using the **real configured `AIProvider`** — never a fixture/mock in the production pipeline. |
| Input | `run(claim: Claim, decision: ClaimDecision) -> ExplanationResult`. Takes the already-produced decision explicitly (not re-derived from `claim`) so the agent structurally cannot compute a different one. |
| Output | `ExplanationResult` (`app/domain/explanation.py`, new in Phase 2D) — `member_summary`, `operations_summary`, `key_reasons`, `deductions`, `policy_findings`, `warnings`, `next_action`, `source: AI \| FALLBACK`, `degraded: bool`, `confidence`, `ai_calls` (empty on fallback, one entry on a real successful call — same convention as `DocumentVerificationResult`/`ClaimExtractionResult`). |
| Evidence sent to the LLM | `_build_evidence()` builds a compact, already-safe JSON dict from `decision` + `claim.policy_evaluation_result`/`financial_calculation_result`/`fraud_analysis_result`/`extraction_result` only — never raw documents, `storage_reference`, full extraction payloads, trace ids, or API keys. Every value is something the member is already entitled to know about their own claim. |
| Prompt constraints | `app/ai/prompts/explanation.py`'s system prompt explicitly instructs the model: never calculate/recalculate amounts, never invent policy rules, never override or contradict `decision`, never restate the approved amount as a different number, never invent facts/dates/names, never claim a document was verified/readable if the evidence says otherwise, never claim fraud unless the fraud evidence says so, and never expose internal implementation details (component names, rule ids, confidence mechanics) in the member-facing text. |
| Structured validation | The raw LLM response (`AIStructuredResponse.data: Dict`) is validated through `ExplanationAIResponse` (Pydantic) before anything is trusted — a well-formed-but-schema-violating response is treated exactly like a provider error (falls back), never partially trusted. |
| Failure handling | **This agent's entire contract is "never raise."** A single `try/except Exception` (deliberately broad — real network/SSL/vendor-SDK failures can surface as exception types this codebase doesn't wrap in `AIProviderError`; verified live against a genuine SSL failure in this environment, see `docs/AI_HANDOFF.md` "Verification (Phase 2D)") wraps evidence-building, the request, the call, and response validation together. On any failure: logs a warning, returns `_fallback()` — a deterministic `ExplanationResult` built directly from `decision`'s own already-computed fields (`member_facing_message`, `explanation`, `financial_breakdown.calculation_steps`, `rejection_reasons`, `fraud_signals`, `degraded_components`), `source=FALLBACK`, `degraded=True`, confidence capped at `min(decision.confidence_score, 0.6)`. Never a generic "something went wrong" placeholder. |
| Trace behavior | One `EXPLANATION` `STARTED`→`COMPLETED` pair — **`COMPLETED` even when the result is a fallback** (the agent itself succeeded at producing *some* valid explanation; `FAILED` is reserved for a defensive-only pipeline-level exception, which should be unreachable given the agent's own internal safety net — see `ClaimsPipeline` below). Metadata: `source`, `degraded`; `ai_metadata` carries the real call's provider/model/latency when `source=AI`. |

---

## ClaimsPipeline — Phase 2D stages (Decision Generation → Explanation)
...
| Property | Value |
|----------|-------|
| New stages | Stage 8 `DECISION_GENERATION`, Stage 9 `EXPLANATION` — both after `FRAUD_ANALYSIS`, both constructor arguments `Optional[...] = None` (same backward-compatibility trick as every prior phase's optional agents — the evaluation runner and every pre-2D pipeline test needed zero changes). |
| Stage 8 semantics | If configured, **always attempted** once the claim has reached this point (i.e. no earlier hard-stop) — not soft-fail-to-`None` like Policy/Financial/Fraud. A genuine exception is caught, `FAILED` is recorded, and `_fallback_decision()` supplies a conservative `MANUAL_REVIEW` decision instead — `claim.decision` is set either way. `claim.status` becomes `ClaimStatus.DECIDED` whenever a decision was actually produced (success or fallback) — the pre-existing `ClaimStatus.DECIDED` value from Phase 0, unused until now. |
| Stage 9 semantics | Only attempted if `claim.decision is not None` (Stage 8 configured and produced a result) **and** `explanation_agent` is configured. `ExplanationAgent.run()` itself never raises (see its own contract above); the pipeline's `try/except` around the call is defense-in-depth only. On success (including a fallback `ExplanationResult` — that's still a *successful* call from the pipeline's point of view), `claim.decision.explanation_detail` is set, and `claim.decision.explanation`/`member_facing_message` are **overwritten** with `explanation_detail.operations_summary`/`member_summary` respectively — so the simple string fields always reflect the best available explanation (AI-crafted when it succeeded, the deterministic fallback otherwise), while `DecisionGenerationAgent`'s own deterministic strings (set at Stage 8) remain in place untouched if Stage 9 never runs or is defensively caught as failed. |
| `claim.status` semantics (updated) | `ClaimStatus.DECIDED` once `claim.decision` is set (Stage 8 ran and produced *any* decision, including the failure-fallback one). `ClaimStatus.PROCESSING` is now reserved for pipelines that reach the end of Stage 7 without `decision_generation_agent` configured at all (pre-2D backward compatibility — e.g. some existing pipeline-level tests deliberately omit it). `ClaimStatus.BLOCKED`/`DOCUMENTS_PENDING` (Phase 2A early-stops) are unaffected — Decision Generation/Explanation are SKIPPED for those claims, never fabricating a decision (`claim.decision` stays `None`). |
| `_PIPELINE_ORDER` extension | `DECISION_GENERATION`/`EXPLANATION` appended to the existing module-level list — the same `_DOWNSTREAM_OF` mapping every Phase 2A early-stop block already uses (Decision 36) automatically marks both new stages `SKIPPED` for any claim that stops earlier, with zero changes needed to those blocks. |
| Persistence | `ClaimORM.decision_json` — one more simple JSON-blob column (claim-level, same pattern as the Phase 2C `*_result_json` columns; includes the nested `explanation_detail` since `ExplanationResult` is itself a field on `ClaimDecision`). `ClaimRepository.save()`/`_to_domain()` extended. `list_by_member()`'s `ClaimHistoryItem.decision` now reads the real persisted `DecisionType` off `row.decision_json["decision"]` instead of always being `None` (a documented Phase 2C gap, closed here). Verified via a genuine **cross-process** round-trip (not just a same-process fresh-repository-read regression test, though that exists too): one Python process ran the real pipeline and saved the resulting claim to an on-disk SQLite file; a completely separate process then loaded it back and confirmed every decision/explanation field matched exactly — see `docs/AI_HANDOFF.md` "Verification (Phase 2D)". |

---

## Claim API — Phase 2D fields

**File**: `app/api/v1/schemas.py`

`ClaimResponse` gained one field: `decision: Optional[ClaimDecision]` — the domain model reused wholesale (same precedent as every other `*_result` field on this schema), rather than flattening `decision.decision`/`.approved_amount`/`.confidence_score`/etc. onto separate top-level `ClaimResponse` fields. `decision.decision`/`.approved_amount`/`.confidence_score`/`.reason_code`/`.explanation` (operations-facing)/`.member_facing_message` (member-facing)/`.explanation_detail` (the full structured `ExplanationResult`) together satisfy every field assignment.md point 4 requires ("every decision must include the approved amount, the reason, and a confidence score"). `None` means "not reached" (claim stopped early — check `status`/`stopped_at`) or "decision generation not configured for this pipeline" — never a fabricated decision, per the explicit requirement that a `BLOCKED` claim's `final_decision` stays `null`.
